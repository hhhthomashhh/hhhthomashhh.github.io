...
short: 'Everyday money, time, travel, health, and shopping calculators.',
url: '/life-calculators/', action: 'Open calculators', featured: true, recent: true,
tags: ['Calculators', 'Finance', 'Travel', 'Health', 'Utilities', 'PWA']
},
{
...
status: 'Live', version: 'v4.0.3', updated: 'Sep 2026', order: 70,
...
const absoluteUrl = project => !project.url ? '' : (/^https?:/.test(project.url) ? project.url : `${root}${project.url}`);
const badge = project => `<div class="card-badges"><span class="status${project.comingSoon ? ' soon' : ''}">${project.status}</span>${project.version ? `<span>${project.version}</span>` : ''}</div>`;
...
const action = project.comingSoon ? '<span class="card-action muted">In development</span>' : `<a class="card-action" href="${href}"${external}>${project.action} <span aria-hidden="true">→</span></a>`;
const favouriteButton = favourite ? `<button class="favourite-button" type="button" data-favourite="${project.id}" aria-label="Add ${project.title} to favourites" aria-pressed="false"><span aria-hidden="true">☆</span></button>` : '';
...
const saveFavourites = () => { try { localStorage.setItem('thomas-playground-favourites', JSON.stringify([...favourites])); } catch (_) {} };
const updateFilterButtons = () => filters.forEach(button => {
...
const project = projects.find(p => p.id === button.dataset.favourite);
button.classList.toggle('selected', selected);
button.setAttribute('aria-pressed', String(selected));
button.setAttribute('aria-label', `${selected ? 'Remove' : 'Add'} ${project.title} ${selected ? 'from' : 'to'} favourites`);
button.querySelector('span').textContent = selected ? '★' : '☆';
});
favouriteToggle.querySelector('span').textContent = favouritesOnly ? '★' : '☆';
favouriteToggle.title = favourites.size ? `${favourites.size} saved favourite${favourites.size === 1 ? '' : 's'}` : 'No favourites saved yet';
};
const render = () => {
...
saveFavourites();
favouritesOnly ? render() : updateFavouriteButtons();
});
